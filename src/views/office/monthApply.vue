<style>
  ...
  /*增加搜索框样式*/
  .monthApply .ivu-icon-search{
    cursor: pointer;
  }


  /*修改表格默认样式*/
 .monthApply .ivu-table-cell{
    padding: 0 !important;
  }
 .monthApply .ivu-input-number-input{
    border: none !important;
    text-align: center;
  }
  .monthApply .ivu-btn-large{
    padding-top: 3px !important;
    padding-bottom: 3px !important;
  }
  .monthApply .ivu-spin-fix{
    background-color: hsla(0,0%,100%,1);
  }

  @media screen and (max-width: 640px) {
      .monthApply .ivu-page-item,.monthApply .ivu-page-item-jump-next,.monthApply .ivu-page-item-jump-prev,.monthApply .ivu-page-next,.monthApply .ivu-page-prev{
        width: 26px;
        height: 26px;
          ...
          <Tabs type="card" v-model="tabVau">
            <Tab-pane label="办公用品申请" name="1">
              <!--办公用品申请-->
                ...
                <!--温馨提示-->
              <p class="tip"><span class="red">小提示：</span>添加成功后还需去<b class="red">我的添加栏</b>确认才能申请审批哦</p>
                <!--table-->
            ...
            <Tab-pane label="劳保用品申请" name="2">
              <!--劳保用品申请-->
                ...
                <!--温馨提示-->
                 <p class="tip"><span class="red">小提示：</span>添加成功后还需去<b class="red">我的添加栏</b>提交才能申请审批哦</p>
                <!--table-->
                <div class="table_page">
                    ...
                    click: (e) => {
                      ...
                      if(numEle){
                        ...
                        if(numEle>0){
                          ...
                          this.confirm("月度需求添加提示","<p>添加编号：<span class='red'>"+params.row.id+"</span></p><p>添加名称：<span class='red'>"+params.row.name+"</span></p><p>添加数量：<span class='red'>"+numEle+"</span></p><p>添加类型：<span class='red'>"+type+"</span></p><p style='color: #f00'>是否添加至待申请栏？</p>",e=>{
                            if(e){
                              this.$api.get('/api.php/index/badd',{"id":params.row.id,"uid":this.$userInfo.id,"type":3,"x_type":params.row.x_type,"num":numEle},r=>{
                                if(r.status==1){
                                  ...
                                  this.$Message.destroy();
                                  this.$Message.error({content:'添加失败',duration:2,closable:true})
                                }
                              },e=>{
                                this.$Message.destroy();
